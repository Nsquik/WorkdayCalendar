...
public static class DateTimeExtensions
{
    ...
    {
        if (calendar.GetRecurringHolidays().Any(rh => rh.Day == date.Day && rh.Month == date.Month)) return false;
    ...
    public static DateTime AddValidWorkdays(this DateTime date, WorkdayCalendarService calendar, int daysToAdd,
        bool direction)
    {
        // daysToAdd can be a negative number
        var counter = Math.Abs(daysToAdd);
        ...
        while (counter > 0)
        {
            ...
            if (date.IsValidWorkday(calendar)) counter -= 1;
        }
    ...
    {
        var isValid = startDate.IsValidWorkday(calendar);
        while (!isValid)
        {
    ...
    public static DateTime AddValidWorkhours(this DateTime startDate, WorkdayCalendarService calendar,
        WorkdayHours workdayHours, bool canAddDirectly, bool direction, double hoursToAdd)
    {
        startDate = startDate.NextValidWorkday(calendar, direction);

        if (canAddDirectly) return startDate.AddHours(hoursToAdd);
